"""Заглушки API — те же контракты, что у core (когда сервер недоступен)."""
...
class MockAgricultureVisionClient:
    ...
    def segment(
        ...
        if architecture == "segformer":
            # Порог влияет на «площадь» mock-полигона, чтобы UI было видно изменение.
            shrink = max(0.05, min(0.45, float(threshold)))
