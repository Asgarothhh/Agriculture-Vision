...
const PORT = process.env.USERS_SERVICE_PORT ? Number(process.env.USERS_SERVICE_PORT) : 5174;
...
app.get('/api/health', async (_req, res) => {
  ...
  res.json({ ok: true, db, ml: SEGMENTATION_SERVICE_URL });
});
...
await waitForDb();
app.listen(PORT, () => {
  console.log(`Users service: http://0.0.0.0:${PORT}`);
  console.log(`User store: ${pool ? 'Postgres' : 'JSON file'}`);
});
