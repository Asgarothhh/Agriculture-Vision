...
_DEFAULT_CONFIG = _SERVICE_ROOT / "config.yaml"
...
class ModelSettings:
    ...
    checkpoint_path: Path = field(
        default_factory=lambda: _SERVICE_ROOT / "weights/best_iou.pth"
    )
...
def load_settings(config_path: str | Path | None = None) -> ModelSettings:
    ...
    if not settings.checkpoint_path.is_absolute():
        settings.checkpoint_path = (_SERVICE_ROOT / settings.checkpoint_path).resolve()
